...
class WebResource(Base):
    ...
    def get(cls, path, site_id, session=DBSession):
        resource = session.query(WebResource).filter(WebResource.uri == path,
            WebResource.website_id == site_id).first()
        return resource
...
class WebSite(Base):
    ...
    local_domain = Column(String(256), nullable=False, unique=True)
    ...
    def get(cls, local_doamin, session=None):
        if session is None:
            session = DBSession()
        site = session.query(WebSite)\
            .filter(WebSite.local_domain == local_doamin).first()
        return site
